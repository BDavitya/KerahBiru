...
class _OrderPageState extends State<OrderPage> {
  ...
  Future<void> _checkAndSelectTimeSlot(String timeSlot, int index) async {
    ...
    try {
      final checkResponse = await ApiService.checkAvailability(
        ...
        timeSlot: availableTimeslots[index],
      );
      ...
      if (checkResponse['available'] == false) {
        ...
        if (mounted) {
          setState(() {
            selectedSession = availableTimeslots[index];
          });
        }
      }
    } catch (e) {
    ...
    try {
      ...
      if (checkResponse['available'] == false) {
        if (mounted) {
          CustomSnackbar.show(
            context,
            message: '❌ Waktu ini baru saja di-booking! Pilih waktu lain.',
            backgroundColor: Colors.red,
                  ...
                  child: selectedDate == null
                      ...
                      : Wrap(
                          ...
                          children: List.generate(
                            convertedTimeslots.length,
                            (index) {
                              final displaySlot = convertedTimeslots[index];
                              final originalSlot = availableTimeslots[index];
                              final isSelected =
                                  selectedSession == originalSlot;
        ...
        child: Padding(
          ...
          child: SizedBox(
            ...
            child: ElevatedButton.icon(
              ...
              onPressed:
                  isSubmitting || isCheckingAvailability ? null : _submitOrder,
            ),
          ),
        ),
      ),
